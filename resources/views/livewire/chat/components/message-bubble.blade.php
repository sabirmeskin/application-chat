<div class="">
    ...
    <div class="flex items-start justify-start gap-2 px-4 py-2">
        ...
        <flux:avatar name="{{$message->sender->name}}" color="auto" class="ml-2 {{ !$avatarOn ? 'opacity-0' : '' }}"
            circle />
        ...
        <flux:dropdown style="display: flex; align-items: center;" class="my-auto" position="top" align="end">
            ...
            <flux:menu>
                ...
                <flux:menu.item icon="copy" x-data="{ messageId: {{ $message->id }} }" x-on:click="navigator.clipboard.writeText(`{{ str_replace(['`', '\\'], ['\`', '\\\\'], $message->body) }}`)
        ...
        <flux:dropdown style="display: flex; align-items: center;">
            ...
            <flux:menu>
                ...
                <flux:menu.item icon="reply" x-data="{ messageId: {{ $message->id }} }"
                    x-on:click="$wire.dispatch('replyMessage', [messageId])">Répondre</flux:menu.item>
                <flux:menu.item icon="copy" x-data="{ messageId: {{ $message->id }} }" x-on:click="navigator.clipboard.writeText(`{{ str_replace(['`', '\\'], ['\`', '\\\\'], $message->body) }}`)
                                $wire.dispatch('copiedMessage', [messageId])">Copier</flux:menu.item>
                <flux:modal.trigger name="delete-profile">
