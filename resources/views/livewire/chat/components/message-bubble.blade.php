<div class="">
    @if ($userId !== $message->sender_id)
        <div class="flex items-start justify-start gap-2 px-4 py-2">
            <flux:avatar name="{{ $message->sender->name }}" color="auto"
                class="ml-2 {{ !$avatarOn ? 'opacity-0' : '' }}" circle />
            ...
            <div class="flex items-start justify-start gap-2 px-4 py-2">
                ...
                <div
                    ...
                    <div class="w-full flex space-x-2 justify-end">
                        <span
                            class="text-sm font-normal text-gray-500 dark:text-gray-400 ">{{ $message->timestamp() }}</span>
                    </div>

                </div>

                <flux:dropdown style="display: flex; align-items: center;" class="my-auto" position="top"
                    align="end">
                    <flux:button icon="ellipsis-vertical" variant="ghost" class="ml-auto mr-2" />
                    <flux:menu>
                        <flux:menu.item icon="forward" x-data="{ messageId: {{ $message->id }} }"
                            x-on:click="$wire.dispatch('forwardMessage', [messageId])">Transférer
                        </flux:menu.item>

                        {{-- <flux:menu.item icon="pencil">Modifier</flux:menu.item> --}}
                        <flux:menu.item icon="reply" x-data="{ messageId: {{ $message->id }} }"
                            x-on:click="$wire.dispatch('replyMessage', [messageId])">Répondre</flux:menu.item>

                        <flux:menu.item icon="copy" x-data="{ messageId: {{ $message->id }} }"
        ...
        @else
            <div class="flex justify-end gap-2 px-4 py-2 ">
                <flux:dropdown style="display: flex; align-items: center;">
                    ...
                    <flux:menu>
                        <flux:menu.item icon="forward" x-data="{ messageId: {{ $message->id }} }"
                            x-on:click="$wire.dispatch('forwardMessage', [messageId])">Transférer</flux:menu.item>
                        <flux:menu.item icon="pencil" x-data="{ messageId: {{ $message->id }} }"
                            x-on:click="$wire.dispatch('editMessage', [messageId])">Modifier</flux:menu.item>
                        <flux:menu.item icon="reply" x-data="{ messageId: {{ $message->id }} }"
                            x-on:click="$wire.dispatch('replyMessage', [messageId])">Répondre</flux:menu.item>
                        <flux:menu.item icon="copy" x-data="{ messageId: {{ $message->id }} }"
                            x-on:click="navigator.clipboard.writeText(`{{ str_replace(['`', '\\'], ['\`', '\\\\'], $message->body) }}`)
                        ...
                        <flux:modal.trigger name="delete-profile">
                            <flux:menu.item variant="danger" icon="trash" x-data="{ messageId: {{ $message->id }} }"
                                x-on:click="$wire.dispatch('confirmDelete', [messageId])">Supprimer</flux:menu.item>
                        </flux:modal.trigger>
