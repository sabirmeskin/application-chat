<div class="h-screen w-full flex flex-col" wire.loading.class="opacity-50">
    @if (!$conversation->isDeletedForUser())
                    ...
                    <flux:avatar.group class="**:ring-zinc-100 dark:**:ring-zinc-800">
                        ...
                        @foreach ($conversation->participants->take(3) as $user)
                            <flux:tooltip content="{{ $user->name }}" placement="top">
                                <flux:avatar circle size="xs" class="max-sm:size-8" name="{{ $user->name }}"
                                    color="auto" badge badge:color="{{ $user->is_online ? 'green' : 'gray' }}"
                                    badge:circle badge:position="top left" badge:variant="xs" />
                            </flux:tooltip>
                        @endforeach
                        @if ($conversation->participants->count() > 3)
                            <flux:tooltip content="{{ $conversation->participants->count() - 3 }} autres"
                                placement="top">
                                <flux:avatar size="xs" circle>{{ $conversation->participants->count() - 3 }}+
                                </flux:avatar>
                            </flux:tooltip>
                        @endif
                    </flux:avatar.group>
            ...
            @else
                <flux:dropdown>
                    ...
                    <flux:menu>
                        ...
                        <flux:menu.item variant="danger" icon="trash" x-data="{ conversationId: {{ $conversation->id }} }"
                            x-on:click="$wire.dispatch('deleteConversation', [conversationId])">Supprimer *
                        </flux:menu.item>
                    </flux:menu>
                </flux:dropdown>
            @endif
        ...
        <div class="flex flex-col h-full  overflow-y-scroll " id="scrollArea" x-init="$nextTick(() => $el.scrollTop = $el.scrollHeight)"
            id="messages-container">
            <div>
                @foreach ($messages as $index => $message)
                    @if ($loop->last)
                        <div x-data="{ observer: null, messageId: {{ $message->id }} }" x-init="observer = new IntersectionObserver((entries) => {
                    ...
                    @if ($message->type === 'media')
                        @unless ($message->deleted_at)
                            <livewire:chat.components.media-message :avatarOn="$index === 0 ||
                                (!empty($messages[$index - 1]) &&
                                    $messages[$index - 1]->sender_id !== $message->sender_id)" :message="$message"
                                :wire:key="'message-'.$message->id" />
                        @endunless
                    @else
                        @unless ($message->deleted_at)
                            <livewire:chat.components.message-bubble :avatarOn="$index === 0 ||
                                (!empty($messages[$index - 1]) &&
                                    $messages[$index - 1]->sender_id !== $message->sender_id)" :message="$message"
                                :wire:key="'message-'.$message->id" />
                        @endunless
...
@if ($editMode)
    <form wire:submit.prevent="updateMessage({{ $editMessageId }})" x-on:submit.debounce.500ms
        ...
        <div class="flex flex-col w-full relative">
            <flux:input placeholder="Modifier votre message" icon-trailing="pencil" clearable wire:model="message"
                id="messageEdit" autocomplete="off" />
        </div>
    ...
    <form wire:submit.prevent="sendMessage" x-on:submit.debounce.500ms
        ...
        <div class="flex flex-col w-full relative">
            ...
            <flux:input placeholder="Taper votre message" icon-trailing="send" clearable wire:model="message"
                id="messageInput" autocomplete="off" wire:keyup.debounce.1000ms="startTyping" />
        </div>
        <flux:button type="submit" variant="primary">
            Envoyer
        </flux:button>
